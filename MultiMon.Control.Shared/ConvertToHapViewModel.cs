...
/// the UI thread through the <see cref="Progress{T}"/> captured synchronization context. No blocking calls
...
public sealed class ConvertToHapViewModel : INotifyPropertyChanged
{
    ...
    private async Task CheckFfmpegAsync()
    {
        ...
        try
        {
            ...
            FfmpegStatus = found
                ? $"FFmpeg found: {path}"
                : "FFmpeg not found — install it (brew install ffmpeg) or place the ffmpeg binary in an 'ffmpeg' folder next to the app.";
        }
        catch (Exception ex)
        {
            FfmpegFound = false;
