...
/// posts and returns; the controller does its work off the UI thread, and its events come back through
/// <see cref="IUiDispatcher"/>. It references only Core and the BCL, so it targets net9.0 and carries no
/// WPF, AppKit or Avalonia type.
/// </summary>
public sealed class MainViewModel : INotifyPropertyChanged
{
    ...
    public MainViewModel(IPerformanceController controller, ILog log, IUiDispatcher ui)
    {
        ...
        // Device enumeration is a call into the audio driver: off the UI thread, populated when it returns.
    ...
    // The controller raises these on its worker thread (the V0087 rule); IsPerforming/Status fire
