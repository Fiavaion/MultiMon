...
| D-008 | `MainViewModel`, `ConvertToHapViewModel`, `FfmpegHapConverter`, `IVideoConverter` exist twice: Windows originals in `MultiMon.Control`/`MultiMon.Platform` and portable copies in `MultiMon.Control.Shared` (net9.0) behind `IUiDispatcher` (Mac TODO 8, `0d546bc`) | **~2 h on the Windows box** | Deliberate: "Windows untouched" was the Mac-port constraint. Dedupe = make `MultiMon.Control` reference `MultiMon.Control.Shared`, delete its copies, adapt the WPF Dispatcher to `IUiDispatcher`, rebuild + `MultiMon.Tests`. Do it in the same Windows pass as D-006. Verified by the TODO 8 review: the copies diverge only at the seam. |
| D-009 | Mac `OnShutdownRequested` (`MultiMon.Control.Mac/App.axaml.cs`) cancels `ShutdownRequested` to run ordered teardown before quitting; acknowledged risk that this may make macOS abort a system Log Out / Shut Down (see ADR `docs/adr/0005-macos-platform-layers.md` §D9) | **~20 min manual test** (log out with the app performing) + a small change if it reproduces (re-post the quit after teardown instead of cancelling) | Untested. |
